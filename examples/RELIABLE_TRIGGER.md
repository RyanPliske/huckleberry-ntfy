...
`examples/push_ntfy_status.py` compares **last bottle** vs **last nursing** and uses the **newer** timestamp as “last feed.” **`FEED_ALERT_AFTER_MINUTES`** (default `150` = 2.5h day) and **`FEED_ALERT_NIGHT_AFTER_MINUTES`** (default `180` = 3h) set how long until the next feed is “due.” Night is **22–07** local (`HUCKLEBERRY_TIMEZONE`) by default (override with **`FEED_ALERT_NIGHT_START_HOUR`** / **`FEED_ALERT_NIGHT_END_HOUR`**). 🟢/🔴 in the title follows **feed timing only** (diaper is shown in the body but does not change red/green). The feed line shows **last feed clock**, **ago**, and **due** (local time = last feed + window); the diaper line uses the same **ago** pattern with a **Diaper** label. Title when overdue: **`🔴 Nancy needs attention`** (default; override with **`FEED_ALERT_TITLE`** → `Nancy · …`); when OK: **`🟢 Nancy ·`** + **`NTFY_TITLE`**. **Priority: urgent** on every run; **`warning`** tag only when feed is overdue.

For **Echo** announcements, enable [Voice Monkey](https://voicemonkey.io/docs), then add repository secrets **`VOICE_MONKEY_TOKEN`** and **`VOICE_MONKEY_DEVICE`**. Alexa runs only when a last feed exists **and** minutes since that feed are **≥** the day/night window **plus** **`VOICE_MONKEY_GRACE_AFTER_FEED_MINUTES`** (default `30`) — so ntfy can be 🔴 first, then Voice Monkey after another 30 minutes past the deadline on each run that still qualifies. Optional env (no GitHub secret required unless you set it in the workflow).
